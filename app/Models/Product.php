...
class Product extends Model
{
    ...
    public function getInwardInvoiceIdsAttribute($value)
    {
        if (is_null($value)) {
            return [];
        }
    ...
    public function setInwardInvoiceIdsAttribute($value)
    {
        if (is_array($value)) {
            $this->attributes['inward_invoice_ids'] = json_encode($value);
        } else {
            $this->attributes['inward_invoice_ids'] = $value;
        }
    }
    ...
    /**
     * Retrieve the barcodes generated for this product.
     *
     * @return HasMany
     */
    ...
    /**
     * Retrieve the design master associated with this product.
     *
     * @return BelongsTo
     */
    public function designMaster(): BelongsTo
    {
     ...
     * Returns unsold barcode count if barcodes exist for this product,
     * otherwise returns the quantity attribute.
     */
    public function getAvailableStockAttribute(): int
    {
        if ($this->relationLoaded('barcodes')) {
            $unsoldBarcodes = $this->barcodes->where('is_sold', 0)->count();
            if ($unsoldBarcodes > 0 || $this->barcodes->count() > 0) {
                return $unsoldBarcodes;
            }
        } elseif ($this->barcodes()->exists()) {
            return (int)$this->barcodes()->where('is_sold', 0)->count();
        }
    ...
    /**
     * Resolve the Canonical Physical Product ID.
     */
    public function getCanonicalIdAttribute(): int
    {
        return app(\App\Services\Accounting\CanonicalInventoryService::class)->resolveCanonicalProductId($this->id);
    }

    /**
     * Sync product quantity column with unsold barcode count if barcodes exist.
     */
    public function syncStockQuantity(): int
    {
        if ($this->barcodes()->exists()) {
            $unsold = (int)$this->barcodes()->where('is_sold', 0)->count();
            $this->update(['quantity' => $unsold]);
            return $unsold;
        }
...
//    }
    public function video(): Attribute
    {
        ...
        if (
            ...
            Storage::disk('public')->exists($this->videoMedia->src) &&
        ...
        */
        if (request()->is('api/*')) {
            ...
            if ($this->videoMedia) {
                ...
                // If video type = file (stored in storage)
                if (
                    $this->videoMedia->type === 'file' &&
                    $symlinkExists &&
                    Storage::disk('public')->exists($this->videoMedia->src)
                    ...
                    $thumbnails->push((object)[
                        'id' => $this->videoMedia->id,
                        'thumbnail' => Storage::disk('public')->url($this->videoMedia->src),
                        'type' => $this->videoMedia->type,
                    ]);
                }

                // If video type = external link (youtube etc.)
                elseif (
                    $this->videoMedia->type !== 'file' &&
                    !empty($this->videoMedia->src)
                ) {
                    $thumbnails->push((object)[
                        'id' => $this->videoMedia->id,
                        'thumbnail' => $this->videoMedia->src,
                        'type' => $this->videoMedia->type,
                    ]);
                }
            }
        ...
        */
        foreach ($this->medias as $media) {
            ...
            if (
                $media &&
                $symlinkExists &&
                Storage::disk('public')->exists($media->src)
            ) {
                $thumbnail = Storage::disk('public')->url($media->src);
            }
...
//    }
    ...
    public function additionalThumbnails(): Collection
    {
        ...
        foreach ($this->medias as $media) {
            ...
            if ($media && $symlinkExists && Storage::exists($media->src)) {
                $thumbnail = Storage::url($media->src);
            }
    ...
    public function hsnMaster(): BelongsTo
    {
        return $this->belongsTo(HsnMaster::class);
    }

    public function vatTax(): BelongsTo
    {
        return $this->belongsTo(VatTax::class);
    }
    ...
    public function scopeIsItemmaster($query,$value)
    {
        // 0 = Product Online Entry 1 = Item Master Entry
        return $query->where('is_item_master',$value);
    }
