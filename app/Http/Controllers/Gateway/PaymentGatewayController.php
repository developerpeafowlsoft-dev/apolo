...
class PaymentGatewayController extends Controller
{
    ...
    public function success(Request $request, Payment $payment)
    {
        ...
        foreach ($payment->orders as $ord) {
            if (!$ord->pos_order) {
                try {
                    app(\App\Services\Accounting\OnlineOrderAccountingService::class)->postOnlineOrderSalesVoucher($ord);
                } catch (\Throwable $e) {
                    \Illuminate\Support\Facades\Log::error('Accounting posting failed on online payment success: ' . $e->getMessage());
                }
            }
        }
