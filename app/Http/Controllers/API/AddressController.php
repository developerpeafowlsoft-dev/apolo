...
class AddressController extends Controller
{
    ...
    {
        // When the user tries to delete an address check if it belongs to logged in user
        if ($address->customer_id != auth()->user()->customer->id) {
            return $this->json('Unauthorized', [], 403);
        }

        // Dissociate from past orders if any so delete won't fail with constraint error
        $address->orders()->update(['address_id' => null]);
        $address->delete();
