...
<head>
    ...
    <!-- App title -->
    <title>{{ $generaleSetting?->title ?? config('app.name', 'Laravel') }}  @hasSection('title') - @yield('title') @endif</title>
        ...
        <div class="app-header">
            ...
            <div class="app-header-content">
                ...
                <div class="app-header-right">
                    ...
                    @if(request()->is('shop/inward-product'))
                        @hasPermission('shop.inwardProduct.index')
                            <div>
                                <div class="d-flex align-items-center flex-wrap gap-3 justify-content-end px-3">
                                    <div>
                                        <button type="button" id="create-record-btn" class="btn py-2 btn-sm btn-primary">
                                            {{ __('Create') }}
                                        </button>
                                        <button type="button" id="list-btn" class="btn py-2 btn-sm btn-primary">
                                            {{ __('Inward List') }}
                                        </button>
                                        <button type="button" id="create-modal-btn" class="btn py-2 btn-sm btn-primary" title="Add Row (F1)">
                                            <i class="bi bi-plus-circle-fill"></i> Add Row (F1)
                                        </button>
                                    </div>
                                </div>
                            </div>
                    ...
                    @if(request()->is('shop/purchase-product'))
                    ...
                    <!-- POS Billing Enter Button -->
                    @if(Route::has('shop.pos.index'))
                        @hasPermission(['shop.pos.index', 'shop.pos.sales', 'shop.pos.draft'])
                            <div class="me-2 me-md-3">
                                <a href="{{ route('shop.pos.index') }}" id="btn-top-pos-billing" class="btn btn-primary btn-sm fw-bold d-inline-flex align-items-center px-3 py-2 text-white shadow-sm" style="border-radius: 8px; background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); border: none; font-size: 13px; transition: all 0.2s ease;">
                                    <i class="fa-solid fa-cash-register me-2" style="font-size: 14px;"></i>
                                    <span>{{ __('POS Billing') }}</span>
                                </a>
                            </div>
                        @endhasPermission
                    @endif

                    <!-- Global Command Spotlight Search Button (Cmd+K) -->
                    <div class="me-2 me-md-3">
                        <button type="button" id="btn-global-command-search" class="btn btn-light btn-sm d-inline-flex align-items-center px-3 py-2 text-secondary border shadow-2xs" style="border-radius: 8px; font-size: 13px; cursor: pointer; background: #ffffff;">
                            <i class="fa-solid fa-magnifying-glass me-2 text-primary"></i>
                            <span class="d-none d-md-inline me-3 fw-medium">{{ __('Search pages, reports...') }}</span>
                            <kbd class="bg-light border text-muted px-1.5 py-0.5 rounded text-xs ms-auto" style="font-size: 11px; font-family: inherit;">⌘K</kbd>
                        </button>
                    </div>
                    ...
                    <div class="badgeButtonBox me-1 me-md-3 dropdown">
                        <div class="notificationIcon">
                            <button type="button" class="emailBadge dropdown-toggle position-relative border-0 bg-transparent p-0 d-flex align-items-center justify-content-center"
                                data-bs-toggle="dropdown" aria-expanded="false" title="{{ __('Shortcuts') }}" id="shortcutDropdownBtn" style="width: 36px; height: 36px; border-radius: 50%;">
                                <i class="fa-solid fa-border-all text-secondary" style="font-size: 17px;"></i>
                            </button>
                            <div class="dropdown-menu dropdown-menu-end p-3 shadow-lg border-0" style="width: 330px; border-radius: 16px; margin-top: 10px; z-index: 1050;">
                                <div class="d-flex justify-content-between align-items-center mb-3 pb-2 border-bottom">
                                    <h6 class="fw-bold m-0 text-dark" style="font-size: 15px;">{{ __('Shortcut') }}</h6>
                                    <button type="button" class="btn btn-light btn-sm rounded-circle p-0 d-flex align-items-center justify-content-center" style="width: 26px; height: 26px;" title="{{ __('Quick Actions') }}">
                                        <i class="fa-solid fa-plus text-muted" style="font-size: 11px;"></i>
                                    </button>
                                </div>
                                <div class="row g-2">
                                    <!-- 1. POS Billing -->
                                    @hasPermission(['shop.pos.index', 'shop.pos.sales', 'shop.pos.draft'])
                                    <div class="col-6">
                                        <a href="{{ Route::has('shop.pos.index') ? route('shop.pos.index') : '#' }}" class="shortcut-item-card d-flex flex-column align-items-center text-center p-3 rounded-3 text-decoration-none h-100">
                                            <div class="shortcut-icon-circle bg-primary-subtle text-primary mb-2 d-flex align-items-center justify-content-center rounded-circle" style="width: 44px; height: 44px; background-color: #eff6ff !important;">
                                                <i class="fa-solid fa-cash-register" style="font-size: 18px;"></i>
                                            </div>
                                            <span class="fw-bold text-dark d-block text-truncate w-100" style="font-size: 13px;">{{ __('POS') }}</span>
                                            <small class="text-muted d-block text-truncate w-100" style="font-size: 11px;">{{ __('Fast Billing') }}</small>
                                        </a>
                                    </div>
                                    @endhasPermission

                                    <!-- 2. Draft / Hold Bills -->
                                    @hasPermission(['shop.pos.index', 'shop.pos.draft'])
                                    <div class="col-6">
                                        <a href="{{ Route::has('shop.pos.draft') ? route('shop.pos.draft') : '#' }}" class="shortcut-item-card d-flex flex-column align-items-center text-center p-3 rounded-3 text-decoration-none h-100">
                                            <div class="shortcut-icon-circle bg-warning-subtle text-warning mb-2 d-flex align-items-center justify-content-center rounded-circle" style="width: 44px; height: 44px; background-color: #fefce8 !important;">
                                                <i class="fa-solid fa-file-invoice" style="font-size: 18px;"></i>
                                            </div>
                                            <span class="fw-bold text-dark d-block text-truncate w-100" style="font-size: 13px;">{{ __('Draft') }}</span>
                                            <small class="text-muted d-block text-truncate w-100" style="font-size: 11px;">{{ __('Hold Orders') }}</small>
                                        </a>
                                    </div>
                                    @endhasPermission

                                    <!-- 3. POS Sales History -->
                                    @hasPermission(['shop.pos.index', 'shop.pos.sales'])
                                    <div class="col-6">
                                        <a href="{{ Route::has('shop.pos.sales') ? route('shop.pos.sales') : '#' }}" class="shortcut-item-card d-flex flex-column align-items-center text-center p-3 rounded-3 text-decoration-none h-100">
                                            <div class="shortcut-icon-circle bg-success-subtle text-success mb-2 d-flex align-items-center justify-content-center rounded-circle" style="width: 44px; height: 44px; background-color: #f0fdf4 !important;">
                                                <i class="fa-solid fa-chart-line" style="font-size: 18px;"></i>
                                            </div>
                                            <span class="fw-bold text-dark d-block text-truncate w-100" style="font-size: 13px;">{{ __('POS Sales') }}</span>
                                            <small class="text-muted d-block text-truncate w-100" style="font-size: 11px;">{{ __('Invoice History') }}</small>
                                        </a>
                                    </div>
                                    @endhasPermission
                                    ...
                                    @hasPermission(['shop.pos.index', 'shop.pos.sales'])
                                    <div class="col-6">
                                        <a href="{{ Route::has('shop.pos.sales') ? route('shop.pos.sales') . '?tab=returns' : '#' }}" class="shortcut-item-card d-flex flex-column align-items-center text-center p-3 rounded-3 text-decoration-none h-100">
                                            <div class="shortcut-icon-circle bg-danger-subtle text-danger mb-2 d-flex align-items-center justify-content-center rounded-circle" style="width: 44px; height: 44px; background-color: #fef2f2 !important;">
                                                <i class="fa-solid fa-rotate-left" style="font-size: 18px;"></i>
                                            </div>
                                            <span class="fw-bold text-dark d-block text-truncate w-100" style="font-size: 13px;">{{ __('POS Return') }}</span>
                                            <small class="text-muted d-block text-truncate w-100" style="font-size: 11px;">{{ __('Product Returns') }}</small>
                                        </a>
                                    </div>
                                    @endhasPermission
                                </div>
    ...
    <script>
        ...
        function changeSvgImageColor(svgImages, svgColor, defaultColor = "#25314C") {
            svgImages.forEach(function(svgImage) {
                ...
                if (!svgPath || svgPath === 'null' || !svgPath.toLowerCase().endsWith('.svg')) {
                    return;
                }
