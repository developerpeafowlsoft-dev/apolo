                ...
                <div class="card mt-3">
                    <div class="card-body">
                        ...
                        <div class="mt-3">
                            ...
                            <select name="category[]" data-placeholder="{{ __('Select Category') }}" class="form-control select2"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ in_array($category->id, old('category', $subCategory->categories?->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-3">
                            <x-input label="Name" name="name" :value="old('name', $subCategory->name)" type="text" placeholder="Name" required="true"/>
                        </div>
                        ...
                        <div class="mt-5 d-flex gap-2 justify-content-between">
                            <a href="{{ route('shop.subcategory.index') }}" class="btn btn-secondary py-2 px-4">
                                {{__('Back')}}
                            </a>
