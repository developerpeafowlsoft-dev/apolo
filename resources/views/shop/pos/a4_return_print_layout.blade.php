...
@php
    ...
    $customerName = $returnHeader->customer?->user?->name ?? 'Walk-in Customer';
    $customerPhone = $returnHeader->customer?->user?->phone ?? '';
    $origInvoiceNo = ($returnHeader->originalOrder->prefix ?? '') . ($returnHeader->originalOrder->order_code ?? '');

    if (empty($qrCodeImage)) {
        try {
            $qrData = route('shop.pos.return.print', $returnHeader->return_no);
    ...
    // Calculate HSN Breakdown for return items
    $hsnSummary = [];

    foreach ($returnHeader->products as $item) {
        ...
        $hsnCode = (string)($product?->hsn ?? $product?->hsn_code ?? $hsnMaster?->hsn_code ?? '950300');

        if ($hsnMaster) {
            $taxRate = (float)$hsnMaster->getTaxForPrice($price);
        } else {
            $taxRate = (float)($product?->vatTax?->percentage ?? $product?->vat_tax_percentage ?? 5.0);
        }
    ...
    <tr>
        ...
        <td style="width: 35%; padding: 0;">
            <table style="width: 100%; border: none;">
                ...
                <tr>
                    <td class="font-bold text-center fz-11" style="background-color: #f2f2f2; border-left: none; border-right: none;">CREDIT NOTE / RETURN RECEIPT</td>
                </tr>
                <tr>
                    <td class="fz-10" style="border-left: none; border-right: none;">
                        <strong>Credit Note No:</strong> #{{ $returnHeader->return_no }}
                    </td>
                </tr>
                <tr>
                    <td class="fz-10" style="border-left: none; border-right: none;">
                        <strong>Orig Invoice:</strong> #{{ $origInvoiceNo }}
                    </td>
                </tr>
                <tr>
                    <td class="fz-10" style="border-bottom: none; border-left: none; border-right: none;">
                        <strong>Date :</strong> {{ $returnHeader->created_at->format('d/m/Y') }}
                    </td>
                </tr>
            </table>
        </td>
...
<table style="width: 100%; margin-top: -1px;">
    <tr>
        ...
        <td style="width: 35%;">
            <strong>Place of Supply :</strong> Unjha, Gujarat
        </td>
    </tr>
</table>
...
<table class="mt-10" style="width: 100%;">
    ...
    <tbody>
        @foreach ($returnHeader->products as $index => $item)
            ...
            @endphp
            <tr>
                <td style="text-align: left;">{{ $product?->name ?? 'Product' }}</td>
    ...
    <tr>
        ...
        <td style="width: 42%; vertical-align: top; border: none; padding: 0;">
            <table style="width: 100%;">
                <tr>
                    <td class="text-right font-bold" style="width: 50%;">Refund Total :</td>
                    <td class="text-right font-bold" style="width: 50%;">{{ number_format($returnHeader->total_amount, 2) }}</td>
                </tr>
                <tr>
                    <td class="text-right font-bold" style="background-color: #f2f2f2;">Total Refund :</td>
                    <td class="text-right font-bold fz-11" style="background-color: #f2f2f2;">{{ number_format($returnHeader->total_amount, 2) }}</td>
                </tr>
                @php
                    $isCash = ($returnHeader->payment_method === 'cash');
                @endphp
                <tr>
                    <td class="text-right">By Cash :</td>
                    <td class="text-right">{{ $isCash ? number_format($returnHeader->total_amount, 2) : '0.00' }}</td>
                </tr>
                <tr>
