                    ...
                    <div class="d-flex gap-3 flex-wrap align-items-center">
                        <div class="flex-grow-1">
                            ...
                            <div class="order-item">
                                ...
                                <span class="value">{{ is_string($order->payment_method) ? $order->payment_method : ($order->payment_method?->value ?? $order->payment_method) }}</span>
                            </div>
                        </div>
            ...
            <div class="card mt-3">
                ...
                @else
                    <div class="p-3 text-muted">
                        {{ __('No shipping address recorded.') }}
                    </div>
                @endif
            </div>
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 16px;">
                <div class="modal-body text-center p-4">
                    ...
                    <div class="d-flex justify-content-center gap-3">
                        ...
                        <form action="{{ route('shop.order.create-shipment', $order->id) }}" method="POST" id="formConfirmCreateShipment" class="m-0">
                            @csrf
    ...
    <script>
        $(document).ready(function () {
            ...
            function fetchCouriers() {
                ...
                $.ajax({
                    url: "{{ route('shop.order.shiprocket.couriers', $order->id) }}",
                    type: "GET",
                    dataType: "json",
            ...
            $(document).on('click', '.btn-assign-courier', function (e) {
                ...
                $.ajax({
                    url: "{{ route('shop.order.shiprocket.assign-courier', $order->id) }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
            ...
            $(document).on('click', '.btn-schedule-pickup', function (e) {
                ...
                $.ajax({
                    url: "{{ route('shop.order.shiprocket.schedule-pickup', $order->id) }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
            ...
            $(document).on('click', '.btn-track-shipment', function (e) {
                ...
                $.ajax({
                    url: "{{ route('shop.order.shiprocket.track', $order->id) }}",
                    type: "GET",
                    dataType: "json",
                    success: function (response) {
