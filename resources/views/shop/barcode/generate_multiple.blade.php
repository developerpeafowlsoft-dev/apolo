...
<div class="page">
    @foreach($barcodeGenerate as $key => $barcode)
        <div class="label">
            <div class="label-content">
                <!-- Product Name -->
                <div class="product-name">
                    {{ Str::limit($barcode->inwardProduct?->products?->name ?? '-', 19) }}
                </div>
                ...
                <div class="size-code-row">
                    <span class="size-part">
                        <span>
                            @if(optional($barcode->inwardProduct)->sizes?->isNotEmpty())
                                {{ $barcode->inwardProduct->sizes->pluck('name')->implode(', ') }}
                            @endif
                        </span>
                    </span>
                    <span class="code-part">
                        {{ $barcode->encoded_price ?? '' }}
                    </span>
                </div>

                <!-- MRP Box -->
                <div class="mrp-box">
                    {{__('Rs')}} {{ number_format($barcode->inwardProduct?->mrp, 2) }}
                </div>

                <!-- Barcode Image -->
                <img src="data:image/png;base64,{{ $barcode->barcode_image }}" class="barcode-img" alt="barcode">
                ...
                <div class="barcode-row">
                    ...
                    <span class="assorte-text">
                        {{ $barcode->inwardProduct->colors->pluck('name')->implode(', ') }}
                    </span>
                </div>
            </div>
        </div>
