    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 my-3 gap-8">
      ...
      <div class="col-span-1 xl:col-span-2">
        ...
        <div
            ...
            class="mt-3 p-3 bg-slate-100 rounded-lg text-slate-600 text-sm"
        >
          {{ $t('Calculating shipping charge') }}...
        </div>

        <div
            v-if="shippingError"
            class="mt-3 p-3 bg-red-50 rounded-lg text-red-600 text-sm"
        >
          {{ shippingError }}
        </div>
        ...
        <div class="p-6 mt-4 bg-white rounded-2xl border border-slate-200">
          ...
          <div class="mt-4 flex flex-wrap gap-4">
            ...
            <label v-if="master.cashOnDelivery && isCodAvailable" for="cash" class="flex items-center gap-4 xl:min-w-80">
              <input v-model="paymentType" id="cash" name="payment" type="radio" class="radioBtn2"
                     value="cash" />
              <div class="p-2 bg-white rounded-xl border border-slate-200">
                <img :src="'assets/icons/money-2.svg'" alt="" class="w-7 h-7">
...
/*
|--------------------------------------------------------------------------
| Cash on Delivery Restriction (India Only)
|--------------------------------------------------------------------------
*/
const isCodAvailable = computed(() => {
  if (!master.cashOnDelivery) return false;
  if (!basketStore.address) return true;

  const country = (
    basketStore.address.country ||
    basketStore.address.country_name ||
    ''
  ).toString().trim().toLowerCase();

  if (!country) {
    return !basketStore.address.country_id || basketStore.address.country_id == 101;
  }

  return country === 'india' || country === 'in';
});

const checkPayment = (master.cashOnDelivery && isCodAvailable.value)
    ? 'cash'
    : 'card';
  ...
  (available) => {
    if (!available) {
      if (paymentType.value === 'cash') {
        if (master.onlinePayment) {
          paymentType.value = 'card';
          paymentMethod.value = paymentGateway.value || null;
        } else {
...
const calculateShippingCharge = async () => {
  if (!basketStore.address?.id) {
    return;
  }

  if (
      !basketStore.checkoutProducts ||
      basketStore.checkoutProducts.length === 0
  ) {
    return;
  }
  ...
  if (
      ...
      basketStore.selectedShopIds.length === 0
  ) {
    return;
  }
  ...
  try {
    ...
    basketStore.delivery_charge = parseFloat(
        response.data.data.delivery_charge || 0
    );

    basketStore.shipping_details =
        response.data.data.shop_shipping || [];

  } catch (error) {
    basketStore.delivery_charge = 0;
    basketStore.shipping_details = [];

    let msg = error.response?.data?.message || 'Shipping charges are temporarily unavailable. Please try again.';
    ...
    if (
      ...
      msg = 'Shipping charges are temporarily unavailable. Please try again.';
    }

    shippingError.value = msg;

    toast.error(shippingError.value, {
      position:
...
/*
|--------------------------------------------------------------------------
| Watch product, shops and address
|--------------------------------------------------------------------------
| Product list, selected shops ya address change ho to shipping recalculate.
*/
...
watch(
    [
      () => basketStore.checkoutProducts,
      () => basketStore.selectedShopIds,
      () => basketStore.address?.id,
    ],
    async ([products, shopIds, addressId]) => {
      if (
          !products ||
          products.length === 0 ||
          !shopIds ||
