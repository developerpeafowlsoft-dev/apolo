...
<!--                &lt;!&ndash; Quantity Increase Or Decrease &ndash;&gt;-->
...
<template>
  <div class="bg-white flex gap-4 w-full group">
    ...
    <div class="grow flex flex-col gap-2 border-b border-slate-200 pb-3 relative overflow-hidden">
      ...
      <!-- ✅ Price - DISCOUNTED PRICE AS MAIN -->
      <div class="flex items-center gap-2 flex-wrap">
        <!-- Main Price (Discounted) - Bold - 460.60 -->
        <div class="text-primary text-base font-bold leading-normal">
          {{ master.showCurrency(parseFloat(props.product?.discount_price || 0).toFixed(2)) }}
        </div>

        <!-- MRP (Strikethrough) - 470.00 -->
        <div v-if="hasMrp" class="text-slate-400 text-sm font-normal line-through leading-tight">
          {{ master.showCurrency(parseFloat(props.product?.price || 0).toFixed(2)) }}
        </div>
      ...
      <div class="flex justify-between gap-2 flex-wrap w-full items-center">
        <div class="flex gap-2 flex-wrap">
          <div v-if="props.product?.size_name && props.product?.size_name !== 'N/A'"
               class="min-w-8 px-2 py-1 bg-slate-100 rounded text-center text-slate-800 text-xs font-normal">
            {{ props.product?.size_name }}
          </div>
          <div v-if="props.product?.color_name && props.product?.color_name !== 'N/A'"
               class="min-w-8 px-2 py-1 bg-slate-100 rounded text-center text-slate-800 text-xs font-normal">
            {{ props.product?.color_name }}
          </div>
        </div>
...
// ✅ Check if MRP exists (for strikethrough)
const hasMrp = computed(() => {
  const discountedPrice = parseFloat(props.product?.discount_price || 0);
  const mrp = parseFloat(props.product?.price || 0);
  return mrp > 0 && mrp > discountedPrice;
});
