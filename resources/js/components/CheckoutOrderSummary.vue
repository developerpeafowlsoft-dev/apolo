  ...
  <div>
    <div class="p-6 bg-white rounded-2xl border border-slate-200">
      ...
      <div class="my-4 flex justify-between gap-4">
        ...
        <div class="text-slate-950 text-base font-normal leading-normal">
          {{ master.showCurrency(basketStore.delivery_charge || 0) }}
        </div>
      </div>

      <!-- Coupon Discount -->
      <div v-if="basketStore.coupon_discount > 0" class="my-4 flex justify-between gap-4">
        <div class="text-green-600 text-base font-normal leading-normal">
          {{ $t('Coupon Discount') }}
        </div>
        <div class="text-green-600 text-base font-bold leading-normal">
...
// ✅ Computed Properties - Using Checkout Data
const checkoutTotalAmount = computed(() => {
  if (basketStore.checkoutProducts && basketStore.checkoutProducts.length > 0) {
    let total = 0;
    basketStore.checkoutProducts.forEach((item) => {
  ...
  return parseFloat(basketStore.totalAmount) || 0;
});
...
const checkoutMrpAmount = computed(() => {
  ...
  return parseFloat(basketStore.totalMrpAmount) || 0;
});
...
const totalPayable = computed(() => {
  ...
  total += parseFloat(basketStore.delivery_charge || 0);
  total -= parseFloat(basketStore.coupon_discount || 0);
  // ❌ REMOVED: tax addition
  return total;
});
...
const processOrderConfirm = () => {
  ...
  if (!basketStore.shipping_details?.length) {
    toast.error("Please calculate shipping charge", {
      position:
...
const fetchCouponApply = () => {
  axios.post("/cart/checkout",
    ...
    basketStore.payable_amount =
        parseFloat(response.data.data.checkout.total_amount || 0)
        + parseFloat(basketStore.delivery_charge || 0)
        - parseFloat(response.data.data.checkout.coupon_discount || 0);
