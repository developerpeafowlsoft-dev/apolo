        ...
        <form @submit.prevent="addressFormSubmit()">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-6 mt-6">
                <div>
                    ...
                    <select id="country" v-model="formData.country_id" class="form-input"
                        :class="errors && errors?.country_id ? 'border-red-500' : 'border-slate-200'">
                        <option value="" disabled>{{ $t('Select Country') }}</option>
                        <option v-for="country in countries" :key="country.id" :value="country.id">
                            {{ country.name }}
                        </option>
                    </select>
                    <span v-if="errors && errors?.country_id" class="text-red-500 text-sm">{{ errors?.country_id[0] }}</span>
                </div>
                <div>
...
const loadCountries = async () => {
    try {
        ...
        if (india && !formData.value.country_id) {
