<template>
  <div>
    ...
    <TransitionRoot as="template" :show="master.basketCanvas">
      <Dialog as="div" class="relative z-[9999]" @close="showCardCanvas()">
        <TransitionChild as="template" enter="ease-in-out duration-500" enter-from="opacity-0"
                         enter-to="opacity-100" leave="ease-in-out duration-500" leave-from="opacity-100"
                         leave-to="opacity-0">
          <div class="fixed inset-0 bg-gray-500 bg-opacity-30 transition-opacity z-[9998]" />
        </TransitionChild>
          ...
          <div class="absolute inset-0 overflow-hidden">
            <div class="pointer-events-none fixed inset-y-0 flex max-w-full"
              ...
              <TransitionChild as="template"
                ...
                <DialogPanel class="pointer-events-auto relative w-screen max-w-md h-full flex flex-col">
                  ...
                  <div class="flex h-full flex-col overflow-hidden bg-white shadow-xl">
                    <div class="p-6 border-b border-slate-200 flex justify-between shrink-0 bg-white">
                    ...
                    <!-- Cart Items -->
                    <div v-if="BasketStore.total > 0" class="relative flex-1 overflow-y-auto py-3 px-4">
                      <Basket />
                    </div>
                    ...
                    <div v-if="BasketStore.total > 0"
                         class="flex gap-6 p-6 border-t border-slate-200 shrink-0 bg-white">
                      <div>
                        <div
                        ...
                        <div
                          ...
                          <!-- ✅ Use totalAmount getter -->
                          {{ master.showCurrency(BasketStore.totalAmount) }}
                        </div>
                        <!-- ✅ Show MRP strikethrough if discount exists -->
                        <div v-if="BasketStore.totalMrpAmount > BasketStore.totalAmount"
                             class="text-slate-400 text-sm line-through text-center">
                          {{ master.showCurrency(BasketStore.totalMrpAmount) }}
                        </div>
                      </div>
                      <button
