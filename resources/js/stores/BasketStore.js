...
export const useBasketStore = defineStore("basketStore", {
    ...
    getters: {
        /**
         * ✅ Total Cart Amount (Using Discounted Price)
         */
        totalAmount: (state) => {
            let total = 0;
        ...
        /**
         * ✅ Total MRP Amount (For strikethrough)
         */
        totalMrpAmount: (state) => {
            let total = 0;
            if (state.products && state.products.length > 0) {
                state.products.forEach((item) => {
                    if (item.products && item.products.length > 0) {
        ...
        totalCheckoutAmount: (state) => {
            let total = 0;
            if (state.checkoutProducts && state.checkoutProducts.length > 0) {
